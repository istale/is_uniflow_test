#!/usr/bin/env bash
# run_cell_hierarchy.sh
# Shell wrapper for cell_hierarchy.py following KLayout JSON I/O standard
set -euo pipefail

KLAYOUT_BIN="/Applications/KLayout.app/Contents/MacOS/klayout"
KLAYOUT_SCRIPT="cell_hierarchy.py"

RAW_ARG="${1:-}"
if [ -z "$RAW_ARG" ]; then
  INPUT_PARAMETER='{"output_file":"cell_hier.gds"}'
elif [ -f "$RAW_ARG" ]; then
